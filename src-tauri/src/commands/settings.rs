...
/// Result of `test_signing_setup`: runs an empty commit with
/// `commit.gpgsign=true` in a scratch repo and reports whether
/// the commit came out signed. The signature status comes from
/// `git log --format='%G?'` (the single-letter code git uses to
/// summarize signature verification).
#[derive(Debug, Serialize, Clone)]
#[serde(rename_all = "camelCase")]
pub struct SigningTestResult {
    pub ok: bool,
    /// Single-letter `git log --format='%G?'` output:
    ///   G = good signature
    ///   B = bad signature
    ///   N = no signature
    ///   E = signature can't be checked
    ///   X = expired signing key
    ///   Y = good signature but expired key
    ///   R = revoked key
    ///   U = good signature with unknown validity
    pub status: String,
    /// Signer email extracted from `%GS`, when present.
    pub signer_email: Option<String>,
    /// Signer key fingerprint extracted from `%GF`, when present.
    pub signer_key_fpr: Option<String>,
    /// Human-readable summary suitable for a toast / status line.
    pub message: String,
}

#[tauri::command]
pub fn test_signing_setup(
    identity_id: String,
    signing_key_id: String,
    signing_key_kind: String,
    ...
    let key = cfg
        .ssh_keys
        .iter()
        .find(|k| k.id == signing_key_id)
        .ok_or_else(|| AppError::NotFound(format!("signing key {}", signing_key_id)))?;

    let scratch = paths::nicessh_dir()?.join("scratch").join("signing-test");
    // Best-effort cleanup of any previous run.
    let _ = fs::remove_dir_all(&scratch);
    paths::ensure_dir(&scratch)?;

    // Init a fresh repo so we can commit without disturbing the
    // user's actual project state.
    let init = runner::exec("git", &["init", "-q", "-b", "main"])?;
    if init.exit_code != Some(0) {
        return Ok(SigningTestResult {
            ok: false,
            status: "E".into(),
            signer_email: None,
            signer_key_fpr: None,
            message: format!("git init failed: {}", init.stderr.trim()),
        });
    }

    // Configure user + signing for this scratch repo. Mirrors what
    // `apply_identity_to_repo` writes for an SSH-style identity so
    // the test exercises the real-world config shape.
    let git_cfg = format!(
        "[user]\n    name = {}\n    email = {}\n    signingkey = {}\n[gpg]\n    format = {}\n[commit]\n    gpgsign = true\n",
        identity.user_name, identity.user_email, key.private_path,
        if signing_key_kind == "gpg" { "openpgp" } else { "ssh" },
    );
    fs::write(scratch.join(".git").join("config"), git_cfg)?;

    // Create an empty commit with signing forced on.
    let commit = runner::exec(
        "git",
        &[
            "-C", scratch.to_string_lossy().as_ref(),
            "commit", "--allow-empty", "-q", "-m", "NiceSSH signing test",
        ],
    )?;
    if commit.exit_code != Some(0) {
        return Ok(SigningTestResult {
            ...
            signer_email: None,
            signer_key_fpr: None,
            message: format!("git commit failed: {}", commit.stderr.trim()),
        });
    }

    // Read back the signature status, signer email, and fingerprint.
    let log = runner::exec(
        "git",
        &[
            "-C", scratch.to_string_lossy().as_ref(),
            "log", "-1", "--format=%G?|%GS|%GF",
        ],
    )?;
    let stdout = log.stdout.trim().to_string();
    let mut parts = stdout.splitn(3, '|');
    let status = parts.next().unwrap_or("E").to_string();
    let signer_email = parts.next().map(str::to_owned).filter(|s| !s.is_empty());
    let signer_key_fpr = parts.next().map(str::to_owned).filter(|s| !s.is_empty());
    let ok = status == "G" || status == "U" || status == "Y";
    let message = match status.as_str() {
        "G" | "U" => format!("Signed by {:?}", signer_email.as_deref().unwrap_or("?")),
        "B" => "Signature present but verification FAILED".into(),
        "X" => "Signing key has expired".into(),
        "Y" => "Signature good, but signing key has expired".into(),
        "R" => "Signing key has been revoked".into(),
        "E" => "git could not check the signature (missing gpg.program?)".into(),
        _ => format!("Commit was not signed (status={status})"),
    };
    // Best-effort cleanup.
    let _ = fs::remove_dir_all(&scratch);
    Ok(SigningTestResult { ok, status, signer_email, signer_key_fpr, message })
}
