...
### Fixed
- [Audit/Git] `clean_repo_gitconfig` and HTTPS re-binding no longer
  preserve or produce a stale `[core] sshCommand` line. The
  decision "should this protocol carry sshCommand?" is
  centralised in `git::protocol::should_emit_sshcommand_for_protocol`
  and consumed by both writers, so future writers cannot drift.
  HTTPS repos with a leftover sshCommand (from a previous SSH
  binding or a hand-edit) now show as `stale-sshcommand-on-https`
  in the Repo Audit table; clicking Clean strips the stale
  line in one click. Regression tests: 9 new unit tests in
  `git/io.rs::tests` and `git/protocol.rs::tests`, plus 2
  integration tests in `tests/audit_https_clean.rs`.
...
- [Rust] Split `src-tauri/src/commands/git.rs` (1850 → 666 lines) into a four-module tree under `src-tauri/src/git/` — `protocol` (reserved), `splice` (pure-string helpers, 7 fns + 22 tests), `bind` (business logic: `apply_identity_to_repo` + `write_repo_remote` + `BindOutcome` + 6 new regression tests), `io` (filesystem wrappers: `write_repo_gitconfig` / `write_repo_user_only` / `clean_repo_gitconfig` + 3 new tests). The `commands::git` module is now a thin shell of 10 `#[tauri::command]` re-exports + 1 `pub(crate)` helper. The 9 new tests cover bind-layer and io-layer combinations that previously had zero coverage and were the source of recurring "修复构建bug" commits (`9677c4d`, `fc09b12`, `f39f93e`, `23779b8`). Public IPC surface and frontend unchanged; `pnpm tsc --noEmit` clean; `cargo test --lib`: 110 pass / 2 fail (2 failures are pre-existing in `commands::settings::tests`, unrelated to this refactor). No user-visible change.

- [UI] History view now has a free-text search box (matches against the human-readable summary) and a row of action-group filter chips (All / Apply / Clean / Repo / SSH Config / Settings). Group mapping is centralised in `src/lib/historyFilter.ts` and covered by 15 unit tests. The empty state distinguishes "no history yet" from "no matches for current filter". 5 new integration tests in `tests/views/HistoryView.test.tsx` cover empty state, search, group filter, no-match fallback, and rollback flow.
### Planned
